...
def generate_pdf(iid:int) -> str:
    ...
    y -= 10*mm
    c.drawString(15*mm, y, "RNC Nº: " + (row.get("rnc_num") or "-"))
    c.drawString(70*mm, y, "Data: " + str(row.get("data") or "-"))
    c.drawString(115*mm, y, "Emitente: " + (row.get("emitente") or "-"))
    y -= 6*mm
    c.drawString(15*mm, y, "Área/Local: " + (row.get("area") or "-"))
    c.drawString(115*mm, y, "Severidade: " + (row.get("severidade") or "-"))
    y -= 6*mm
    c.drawString(15*mm, y, "PEP: " + (row.get("pep") or "-"))
    c.drawString(115*mm, y, "Categoria: " + (row.get("categoria") or "-"))
    ...
    y = draw_block("Encerrado por / Em:", (row.get("encerrada_por") or "-") + " / " + str(row.get("encerrada_em") or "-"), y - 6*mm)
    ...
    y = draw_block("Reaberta por / Em:", (row.get("reaberta_por") or "-") + " / " + str(row.get("reaberta_em") or "-"), y - 6*mm)
...
if menu == "Nova RNC":
    ...
    with st.form("form_rnc"):
        ...
        if submitted:
            ...
            st.success("RNC salva! Nº " + rnc_num + " • Código interno: #" + str(iid))
    ...
    if not df.empty:
        ...
        if not df.empty:
            ...
            if sel_id in df["id"].values:
                ...
                st.subheader("RNC Nº " + (row.get("rnc_num") or "-") + " — " + row.get("titulo","-") + " [" + row.get("status","-") + "]")

                if st.button("📄 Gerar PDF desta RNC", key="pdf_" + str(sel_id)):
                    path = generate_pdf(int(row.get("id")))
                    with open(path, "rb") as _f:
                        st.download_button("Baixar PDF", _f, file_name="RNC_" + (row.get("rnc_num") or str(row.get("id"))) + ".pdf", mime="application/pdf", key="dl_" + str(sel_id))

                c1, c2, c3, c4, c5, c6 = st.columns(6)
                c1.metric("Data", str(row.get("data","-")))
                c2.metric("Severidade", row.get("severidade","-"))
                c3.metric("Status", row.get("status","-"))
                c4.metric("PEP", row.get("pep","-"))
                c5.metric("RNC Nº", row.get("rnc_num","-"))
                c6.metric("Emitente", row.get("emitente","-"))
                st.write("**Área/Local:**", row.get("area","-"))
                st.write("**Resp. inspeção:**", row.get("responsavel","-"))
                st.write("**Resp. ação corretiva:**", row.get("responsavel_acao","-"))
                st.write("**Categoria:**", row.get("categoria","-"))
                st.markdown("**Descrição**")
                st.write(row.get("descricao","-"))
                st.markdown("**Referências**")
                st.write(row.get("referencias","-"))
                st.markdown("**Causador / Processo envolvido / Origem**")
                st.write("- **Causador:**", row.get("causador","-"))
                st.write("- **Processo:**", row.get("processo_envolvido","-"))
                st.write("- **Origem:**", row.get("origem","-"))
                st.markdown("**Ação de correção**")
                st.write(row.get("acao_correcao","-"))

                tabs = st.tabs(["📸 Abertura", "✅ Encerramento", "♻️ Reabertura", "🗂️ Cancelamento / Exclusão"])
                with tabs[0]:
                    for rec in fetch_photos(int(row.get("id")), "abertura"):
                        try: st.image(rec.get("url"), width=360)
                        except: st.caption("Foto indisponível")
                with tabs[1]:
                    enc = fetch_photos(int(row.get("id")), "encerramento")
                    st.markdown("**Observações de encerramento:**")
                    st.write(row.get("encerramento_obs","-"))
                    st.markdown("**Descrição do fechamento:**")
                    st.write(row.get("encerramento_desc","-"))
                    ...
                    if st.session_state.is_quality and row.get("status") != "Encerrada":
                        st.markdown("---")
                        with st.form("encerrar_" + str(sel_id)):
                            ...
                            fotos_enc = st.file_uploader("Evidências (fotos)", type=["jpg","jpeg","png"], accept_multiple_files=True, key="enc_" + str(sel_id))
                            sub = st.form_submit_button("Encerrar RNC")
                            if sub:
                                meta = storage_upload_images(fotos_enc, "encerramento", row.get("rnc_num") or str(row.get("id")))
                                encerrar_inspecao(int(row.get("id")), encerr_por.strip(), encerr_obs.strip(), efficacy_map(eficacia), encerr_desc.strip(), meta)
                                st.success("RNC encerrada. Recarregue para ver o novo status.")
                with tabs[2]:
                    rea = fetch_photos(int(row.get("id")), "reabertura")
                    st.markdown("**Motivo da reabertura:**")
                    st.write(row.get("reabertura_motivo","-"))
                    st.markdown("**Descrição da reabertura:**")
                    st.write(row.get("reabertura_desc","-"))
                    if rea:
                        st.markdown("**Evidências:**")
                        for rec in rea:
                            try: st.image(rec.get("url"), width=360)
                            except: st.caption("Foto indisponível")
                    if st.session_state.is_quality and row.get("status") in ["Encerrada","Em ação","Em análise","Bloqueada","Aberta"]:
                        st.markdown("---")
                        with st.form("reabrir_" + str(sel_id)):
                            ...
                            fotos_reab = st.file_uploader("Fotos (opcional)", type=["jpg","jpeg","png"], accept_multiple_files=True, key="reab_" + str(sel_id))
                            sub2 = st.form_submit_button("Reabrir RNC")
                            if sub2:
                                meta = storage_upload_images(fotos_reab, "reabertura", row.get("rnc_num") or str(row.get("id")))
                                reabrir_inspecao(int(row.get("id")), reab_por.strip(), reab_motivo.strip(), reab_desc.strip(), meta)
                ...
                with tabs[3]:
                    if st.session_state.is_quality:
                        ...
                        with st.form("cancel_" + str(sel_id)):
                            ...
                            if do_cancel:
                                cancel_inspecao(int(row.get("id")), c_por.strip(), c_motivo.strip())
                                st.success("RNC marcada como Cancelada.")
                        st.divider()
                        with st.form("delete_" + str(sel_id)):
                            ...
                            if do_delete and confirm.strip().upper() == "CONFIRMAR":
                                delete_inspecao(int(row.get("id")))
        ...
        if up is not None:
            ...
            with engine.begin() as conn:
                ...
                for _, row in df_imp.iterrows():
                    ...
                    n += 1
            st.success(str(n) + " registro(s) importado(s)/atualizado(s).")
